...
export function ArticleCard({ postDetails, preview, userId }: ArticleCardProps) {
  dayjs.extend(customParseFormat);
  const { mutate: deletePost } = useDeletePost((postDetails?.type === POST_TYPE.EVENT ||
    postDetails?.type === POST_TYPE_NAME.EVENT) ? POST_TYPE.EVENT : POST_TYPE.FEED);
  return (
    <Card className='w-full p-4 shadow-md rounded-lg flex flex-col gap-4 mb-4'>
      <div className="flex justify-between">
        ...
        {!preview && userId === postDetails.author.id && <DeleteModal
          handleOnDelete={() => { deletePost(postDetails.id) }}
          trigger={<Button
